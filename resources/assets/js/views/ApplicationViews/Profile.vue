    ...
    <article class="content-block-profil">
        ...
        <div class="col-md-6">
            <form action="" @submit.prevent="saveProfile" @keydown="deleteErrors($event.target.name)">
                ...
                <div class="row select-dropdown" style="margin-bottom: 0">
                    <select class="application-select" id="application-select" v-model="special_preparation" @change="changeDropDown">
                        <option value="0" disabled style="color: #000; font-weight: 600;">Hvilken specialistuddannelse ønsker du?</option>
                        <option class="default-option" :value="g.id" v-for="g in groups" :class="{ bold:  ! g.name }" :disabled=" ! g.name" :selected="special_preparation == g.id">
                           <span v-if=" ! g.name">{{ g }}</span>
                           <span v-if="g.name">{{ g.name }}</span>
                        </option>
                    </select> 
                </div>
...
//        Collect information to vue.data
        data()
    {
        return {
            ...
            city: '',
            phone: '',
            special_preparation: 'Psykoterapi med voksne',
            sp_text : '',
            requires : [],
            options: [
                {text: 'Børnepsykologi: ', value: 1},
                {text: 'Voksenpsykologi: ', link: 6},
                {text: 'Arbejds- og organisationspsykologi: ', link: 9},
            ],
            groups : [],
            user_avatar: '',
            user_role: '',
    ...
    created: function () {
        ...
        axios.get('/informationAboutUser')
                .then(function (response) {
                    ...
                    that.requires = response.data.requires;
    ...
    ,
        saveProfile()
        {
            ...
            axios.post('/updateInformationUser', data)
                    .then(function (response) {
                        swal('Godt arbejde!', 'Din profil opdateret!', 'success').then(function(){
